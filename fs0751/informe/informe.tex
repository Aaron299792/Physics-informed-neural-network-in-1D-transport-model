...
\documentclass[aps,twocolumn,secnumroman ,nobalancelastpage,amsmath,amssymb,
nofootinbib]{revtex4}
...
\subsection{Modelo de transporte clásico}
...
\subsection{Simulaciones con diferentes parámetros}

Con la PINN implementada para resolver el sistema de ODEs se procede a hacer pruebas con diferentes parámetros para estudiar su impacto y sensibilidad en las soluciones. En la Figura \ref{fig:resden} se tienen los residuos de la ODE de partículas, así como los diferentes perfiles de densidad electrónica calculados a partir de parámetros físicos típicos del RTP obtenidos de \cite{donne1994, roach2008}. Mismos que se presentan en la Tabla \ref{tab:physparams} que se usaron para todos los cálculos realizados.
...
En la Figura \ref{fig:resden} se puede observar que no hubo mayor impacto en el cambio de parámetros. Los residuos se mantienen en un orden similar, aunque al aumentar la profundidad de la red con $L = 128$ aumentó el valor de los residuos para todos los casos. Así mismo, al observar el panel (d) se puede ver que la gran mayoría de pruebas llegaron al mismo perfil de densidad. 

\begin{figure*}[t] % [t] forces it to the top of the page, which is standard for full-width figures
  \centering
  \begin{minipage}{0.497\linewidth}
    \centering
    \includegraphics[width=\linewidth]{../../plots/resd123.pdf}
    \vspace{1mm}
    {(a) Parámetros de red $N_{c,1}$, $w_1$}
  \end{minipage}
  \hfill
  \begin{minipage}{0.497\linewidth}
    \centering
    \includegraphics[width=\linewidth]{../../plots/resd456.pdf}
    \vspace{1mm}
    {(b) Parámetros de red $N_{c,2}$, $w_2$}
  \end{minipage}
  \hfill
    \begin{minipage}{0.497\linewidth}
    \centering
    \includegraphics[width=\linewidth]{../../plots/resd789.pdf}
     \vspace{1mm}
     {(c) Parámetros de red $N_{c,3}$, $w_3$}
  \end{minipage}
  \hfill
  \begin{minipage}{0.497\linewidth}
    \centering
    \includegraphics[width=\linewidth]{../../plots/dens.pdf}
     \vspace{1mm}
     {(d) Parámetros de red con menor residuo}
  \end{minipage}
  \caption{Residuos de la función de pérdida para el ODE de partículas para diferentes números de capas ocultas $L$ para la PINN con:  (a) con puntos de colocación $N_{c,1}$ y pesos $w_1$, (b) con puntos de colocación $N_{c,2}$ y pesos $w_2$, (c) con puntos de colocación $N_{c,3}$ y pesos $w_3$ y (d) perfiles de densidad electrónica con distintos parámetros de la PINN.}
  \label{fig:resden}
\end{figure*}
...
\begin{figure*}[t] % [t] forces it to the top of the page, which is standard for full-width figures
  \centering
    \begin{minipage}{0.497\linewidth}
    \centering
    \includegraphics[width=\linewidth]{../../plots/rest123.pdf}
     \vspace{1mm}
    {(a) Parámetros de red $N_{c,1}$, $w_1$}
  \end{minipage}
  \hfill
  \begin{minipage}{0.497\linewidth}
    \centering
    \includegraphics[width=\linewidth]{../../plots/rest456.pdf}
     \vspace{1mm}
    {(b) Parámetros de red $N_{c,2}$, $w_2$}
  \end{minipage}
  \hfill
    \begin{minipage}{0.497\linewidth}
    \centering
    \includegraphics[width=\linewidth]{../../plots/rest789.pdf}
     \vspace{1mm}
    {(c) Parámetros de red $N_{c,3}$, $w_3$}
  \end{minipage}
  \hfill
  \begin{minipage}{0.497\linewidth}
    \centering
    \includegraphics[width=\linewidth]{../../plots/temps.pdf}
     \vspace{1mm}
    {(d) Perfiles de temperatura PINN}
  \end{minipage}
  \caption{Residuos de la función de pérdida para el ODE de energía para diferentes números de capas ocultas $L$ para la PINN con:  (a) con puntos de colocación $N_{c,1}$ y pesos $w_1$, (b) con puntos de colocación $N_{c,2}$ y pesos $w_2$, (c) con puntos de colocación $N_{c,3}$ y pesos $w_3$ y (d) perfiles de temperatura electrónica con distintos parámetros de la PINN.}
\end{figure*}
...
\begin{figure*}[t] % [t] forces it to the top of the page, which is standard for full-width figures
  \centering
    \begin{minipage}{0.497\linewidth}
    \centering
    \includegraphics[width=\linewidth]{../../plots/rtp_den.pdf}
     \vspace{1mm}
    {(a)}
  \end{minipage}
  \hfill
  \begin{minipage}{0.497\linewidth}
    \centering
    \includegraphics[width=\linewidth]{../../plots/results_den.pdf}
     \vspace{1mm}
    {(b)}
  \end{minipage}
  \hfill
    \begin{minipage}{0.497\linewidth}
    \centering
    \includegraphics[width=\linewidth]{../../plots/rtp_temp.pdf}
     \vspace{1mm}
    {(c)}
  \end{minipage}
  \hfill
  \begin{minipage}{0.497\linewidth}
    ...
    {(d)}
  \end{minipage}
  \caption{Perfiles de densidad y temperatura electrónica. En la columna izquierda se tienen las mediciones de descargas del RTP para densidad (a) y temperatura (d). En la columna derecha perfiles obtenidos por la PINN contra promedios de descarga para densidad  y ajuste (b) y para temperatura (d). Datos obtenidos de the ITPA International Multi-Tokamak Confinement Profile Database \cite{roach2008}, bajo libre acceso.}
\end{figure*}

\begin{table}[]
\caption{Parámetros de la PINN usados para la obtención de los perfiles de densidad y temperatura electrónica.}
\label{tab:net_param}
...
$L$                & Capas ocultas              & $32$                                 \\
$N_c$              & Puntos de Colocación       & $1000$                               \\
$N_b$              & Puntos en la frontera      & $1$                                  \\
$N_t$              & Puntos de prueba           & $300$                                \\
$w_i$              & Pesos de pérdida           & $[0.1, 0.001, 1.0, 1.0, 0.01, 0.01]$ \\
its0               & Iteraciones de preparación & $2000$                               \\
its                & Iteraciones de Adam        & $7000$                               \\
$x_\text{eval}$    & Puntos de observación      & $40$                                 \\ \hline
\end{tabular}}
\end{table}
...
\begin{figure}[ht!]
	...
	\caption{Residuos de la función de pérdida para el ODE de partículas y energía energía para los parámetros usados que mejor se ajustan a las mediciones.}
...
\begin{table}[]
\caption{MSE calculado para distintos parámetros de la PINN respecto al promedio de los datos medidos en el RTP. Datos obtenidos de the ITPA International Multi-Tokamak Confinement Profile Database \cite{roach2008}, bajo libre acceso.}
\label{tab:mse}
\begin{tabular}{ccc}
\hline
\multicolumn{1}{l}{Parámetros} & \multicolumn{1}{l}{MSE Densidad} & \multicolumn{1}{l}{MSE Temperatura} \\ \hline
$\text{L}_1$, $N_{c,1}$, $w_1$ & 0.049442 & 0.022991 \\
$\text{L}_2$, $N_{c,1}$, $w_1$ & 0.049955 & 0.022830 \\
$\text{L}_3$, $N_{c,1}$, $w_1$ & 0.048319 & 0.022528 \\
$\text{L}_1$, $N_{c,2}$, $w_2$ & 0.048447 & 0.023023 \\
$\text{L}_2$, $N_{c,2}$, $w_2$ & 0.049100 & 0.022951 \\
$\text{L}_3$, $N_{c,2}$, $w_2$ & 0.051410 & 0.022620 \\
$\text{L}_1$, $N_{c,3}$, $w_3$ & 0.050697 & 0.021823 \\
$\text{L}_2$, $N_{c,3}$, $w_3$ & 0.052272 & 0.021810 \\
$\text{L}_3$, $N_{c,3}$, $w_3$ & 0.115087 & 0.021799 \\ \hline
\end{tabular}
\end{table}
